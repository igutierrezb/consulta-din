...
function assignmentGroup_(s,a){const gs=s.data.GRUPOS.filter(g=>g.periodo===a.periodo),byId=gs.filter(g=>a.id_grupo&&g.id_grupo===a.id_grupo);if(byId.length)return byId.length===1?byId[0]:null;const code=a.grupo||a.id_grupo,found=gs.filter(g=>code&&normalizar_(g.grupo)===normalizar_(code));return found.length===1?found[0]:null;}
function assignmentRoom_(s,a){const byId=s.data.AULAS.filter(r=>a.id_aula&&r.id_aula===a.id_aula);if(byId.length)return byId.length===1?byId[0]:null;const name=a.salon||a.nombre||a.aula;if(!name)return null;const found=s.data.AULAS.filter(r=>normalizar_(r.nombre||r.salon||r.aula)===normalizar_(name)&&(!a.planta||normalizar_(r.planta).replace(/^planta /,'')===normalizar_(a.planta).replace(/^planta /,''))&&(!a.edificio||(resolveBuilding_(s,a.edificio)&&resolveBuilding_(s,a.edificio)?.id_edificio===resolveBuilding_(s,r.edificio)?.id_edificio)));return found.length===1?found[0]:null;}
function repairReferences_(s){
 s.data.AULAS.forEach(r=>{if(!r.nombre&&(r.salon||r.aula))r.nombre=String(r.salon||r.aula);const b=resolveBuilding_(s,r.edificio);if(b)r.edificio=b.id_edificio;});
 let previous,readPrevious=false;
 s.data.ASIGNACION_AULAS.forEach(a=>{const b=resolveBuilding_(s,a.edificio);if(b)a.edificio=b.id_edificio;const g=assignmentGroup_(s,a);if(g)a.id_grupo=g.id_grupo;let room=assignmentRoom_(s,a);
  if(!room&&a.id_aula){if(!readPrevious){readPrevious=true;try{previous=release_();}catch{previous=null;}}const old=previous?.data?.AULAS?.filter(r=>r.id_aula===a.id_aula)||[];if(old.length===1){const r=old[0],oldBuilding=resolveBuilding_(previous,r.edificio);room=assignmentRoom_(s,{salon:r.nombre||r.salon||r.aula,planta:r.planta,edificio:oldBuilding?.id_edificio||r.edificio});}}
  if(room)a.id_aula=room.id_aula;
 });return s;
}
...
function validateState_(s){
 ...
 s.data.ASIGNACION_AULAS.filter(a=>a.periodo===s.active&&active(a.activo)).forEach((a,i)=>{
  ...
  if(((a.grupo||a.id_grupo)&&groups.length!==1)||rooms.length!==1)errors.push(groups.length===1?'Revisa el aula de '+groups[0].grupo+'.':'Hay asignaciones sin grupo reconocido. Corrígelas en Excel y datos → Asignaciones.');
 });
