...
(()=>{
 ...
 function draw(){
  const shapes=areas.map((a,i)=>!a.points?'':`<polygon class="campus-hotspot ${a.reference?'reference':''} ${selected===i?'selected':''}" points="${a.points}" role="button" tabindex="0" data-campus-area="${i}" aria-label="${esc(a.name)}, consultar edificio" aria-pressed="${selected===i}"><title>${esc(a.name)}</title></polygon>`).join('');
  const labels=areas.filter(a=>a.label).map(a=>`<g class="campus-label" pointer-events="none" transform="translate(${a.label.x} ${a.label.y}) rotate(-23)"><rect x="${-a.label.width/2}" y="-24" width="${a.label.width}" height="48" rx="6" fill="#a32134" stroke="#fff" stroke-width="2"/><text text-anchor="middle" dominant-baseline="middle" fill="#fff" font-family="system-ui,sans-serif" font-size="30" font-weight="800">${esc(a.label.text)}</text></g>`).join('');
  $('campusCanvas').innerHTML='<svg viewBox="0 0 1408 1787" xmlns="http://www.w3.org/2000/svg" role="group" aria-label="Mapa del campus UTEQ"><image href="campus-uteq.jpg" width="1408" height="1787" preserveAspectRatio="none"/>'+shapes+labels+'</svg>';
