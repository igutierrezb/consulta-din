...
function refreshModel(id){
 ...
 render();if(['profesores','grupos'].includes(mode))DIN_SCHEDULE.mount(mode,$('horarios'));
}
...
function render(){
 ...
 if(!query&&!$('career').value){$('count').textContent='';$('results').innerHTML=empty(mode==='tutors'?'Encuentra a tu tutor':'Tu grupo, tu tutor y tu aula','Busca por grupo, nombre, carrera o correo.');return;}
 const groups=matchingGroups(query);
 if(mode==='groups'){$('count').textContent=groups.length+' grupos';$('results').innerHTML=groups.length?groups.map(groupCard).join(''):empty('Sin coincidencias','Prueba otro código, nombre, carrera o correo.');return;}
...
function setMode(value){
 ...
 $('hint').textContent=value==='rooms'?'Todos los planos vigentes. Busca un grupo para resaltar su salón en amarillo.':'La búsqueda ignora acentos y mayúsculas.';render();if(['profesores','grupos'].includes(mode))DIN_SCHEDULE.mount(mode,$('horarios'));
}
...
function drawMap(){
 ...
 const svg=spaces.map(s=>{
  ...
  const fill=selected?'#ffdf57':r?'#dedede':s.kind==='corridor'?'#eee':s.kind==='desk'?'#c7c7c7':'#e5e5e5';
