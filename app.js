...
function render(){
 ...
 if(mode==='groups'){const careers=[...new Set(model.groups.map(g=>str(g.ingenieria)))].sort(DIN.naturalOrder);$('count').textContent=groups.length+' grupos';$('results').innerHTML=groups.length?'<div class="group-picker" aria-label="Selecciona tu grupo">'+groups.map(g=>`<button type="button" class="group-pick career-tone-${careers.indexOf(str(g.ingenieria))%8}" data-group="${esc(g.id_grupo)}"><strong>${esc(g.grupo)}</strong><small class="group-pick-career">${esc(g.ingenieria||'Carrera pendiente')}</small><small class="group-pick-exit">${esc(g.salida_lateral||'')}</small>${g.cuatrimestre?`<small class="group-pick-term">${esc(g.cuatrimestre)}º cuatrimestre</small>`:''}${DIN.hasTutor(g.tutor)?`<small class="group-pick-tutor">Tutor: ${esc(g.tutor)}</small>`:''}</button>`).join('')+'</div><div id="groupDetail" class="group-detail"></div>':empty('Sin coincidencias','Prueba otro código, nombre, carrera o correo.');return;}
 const names=[...new Set(groups.filter(g=>DIN.hasTutor(g.tutor)).map(g=>norm(g.tutor)))].sort(DIN.naturalOrder);
 ...
 $('results').innerHTML=names.length?names.map(name=>{
  ...
  return `<details class="tutor-card"><summary><span>${esc(all[0].tutor)}</span><span class="teacher-tutor-icon" aria-label="Tutor">♟</span></summary><div class="tutor-card-content"><p>${emails.length?emails.map(e=>`<a href="mailto:${esc(e)}">${esc(e)}</a>`).join('<br>'):'Correo pendiente de captura'}</p><p class="hint">Todos sus grupos del periodo, incluidas otras carreras.</p><div class="tutor-groups">${all.map(g=>`<div><b>${esc(g.grupo)}</b><span>${esc(g.ingenieria)}</span>${model.placements(g).map(p=>`<small>${esc(DIN.buildingLabel(p.building))} · ${esc(p.room.planta)} · ${esc(model.roomLabel(p.room))}</small>${mapButton(p.room)}`).join('')}</div>`).join('')}</div></div></details>`;
...
function openMap(buildingId,roomId){
 ...
 const plans=window.DIN_PLANOS.filter(p=>p.edificio===buildingId).sort((a,b)=>DIN.floorOrder(a.planta,b.planta));const b=model.buildings.find(b=>b.id_edificio===buildingId);if(!b)return;
 currentMap=geo?.plan||plans[0]||null;chosenRoom=room;scale=1;opener=document.activeElement;$('mapTitle').textContent=DIN.buildingLabel(b);
...
function drawRoomDetail(){
 ...
 $('roomDetail').innerHTML=`<h3>${esc(model.roomLabel(chosenRoom))}</h3><div>${esc(DIN.buildingLabel(model.building(chosenRoom.edificio)))} · Planta ${esc(norm(chosenRoom.planta))}${chosenRoom.capacidad?' · '+esc(chosenRoom.capacidad)+' personas':''}</div>${groups.length?groups.map(({g,p})=>`<div><b>${esc(g.grupo)}</b> · ${esc(p.assignment.turno||'Sin turno registrado')}${DIN.hasTutor(g.tutor)?' · '+esc(g.tutor):''}</div>`).join(''):'<div>Sin grupos asignados en este periodo.</div>'}${chosenRoom.observaciones?'<div>'+esc(chosenRoom.observaciones)+'</div>':''}`;
}
