...
function render(){
 ...
 if(mode==='groups'){$('count').textContent=groups.length+' grupos';$('results').innerHTML=groups.length?'<div class="group-picker" aria-label="Selecciona tu grupo">'+groups.map(g=>`<button type="button" class="group-pick" data-group="${esc(g.id_grupo)}"><strong>${esc(g.grupo)}</strong><small>${esc(g.ingenieria||'Ver grupo')}</small>${g.cuatrimestre?`<small class="group-pick-term">${esc(g.cuatrimestre)}º cuatrimestre</small>`:''}${DIN.hasTutor(g.tutor)?`<small class="group-pick-tutor">Tutor: ${esc(g.tutor)}</small>`:''}</button>`).join('')+'</div><div id="groupDetail" class="group-detail"></div>':empty('Sin coincidencias','Prueba otro código, nombre, carrera o correo.');return;}
 ...
 $('results').innerHTML=names.length?names.map(name=>{
  ...
  return `<article class="group-card"><h3>${esc(all[0].tutor)}</h3><p>${emails.length?emails.map(e=>`<a href="mailto:${esc(e)}">${esc(e)}</a>`).join('<br>'):'Correo pendiente de captura'}</p><p class="hint">Todos sus grupos del periodo, incluidas otras carreras.</p><div class="tutor-groups">${all.map(g=>`<div><b>${esc(g.grupo)}</b><span>${esc(g.ingenieria)}</span>${model.placements(g).map(p=>`<small>${esc(DIN.buildingLabel(p.building))} · ${esc(p.room.planta)} · ${esc(model.roomLabel(p.room))}</small>${mapButton(p.room)}`).join('')}</div>`).join('')}</div></article>`;
